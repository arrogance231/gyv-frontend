  ...
  const items = [
    {
      ...
      date: '2025-11-12',
      slug: 'open-letter-to-ministers',
      summary: 'A formal government letter requesting urgent policy action on youth tobacco use.',
    },
    {
      title: 'Statement on Flavored Products',
      date: '2026-02-03',
      slug: 'statement-on-flavored-products',
      summary: 'An official communication calling for prohibition of youth-targeted flavored tobacco products.',
    },
  ];
      ...
      <div className="mt-6 space-y-4">
        {items.map((it) => (
          <article key={it.title} className="rounded-lg border border-[#1D5975]/10 bg-white p-4 shadow-sm">
            ...
            <Link href={`/resources/official-communications/${it.slug}`} className="mt-3 inline-block text-sm font-semibold text-[#1282AA]">
              Read
            </Link>
          </article>
        ))}
